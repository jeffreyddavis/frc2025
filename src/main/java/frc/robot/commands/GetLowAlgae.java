...
public class GetLowAlgae extends SequentialCommandGroup {

  public GetLowAlgae(Arm arm, Intake intake, Elevator elevator) {

    addCommands(
        Commands.runOnce(() -> arm.goToLocation(Constants.Arm.SafeCarryAngle), arm),
        Commands.waitUntil(() -> arm.isAtLocation()),
        Commands.runOnce(() -> elevator.goToLocation(Constants.Elevator.HeightAlgaeLow), elevator),
